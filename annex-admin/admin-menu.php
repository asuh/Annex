...
/*
	...
	Description: A merger created by Aaron T. Grogg (<a href="http://aarontgrogg.com/">http://aarontgrogg.com/</a>)
		...
		and the Twenty Eleven theme (<a href="http://wordpress.org/extend/themes/twentyeleven">http://wordpress.org/extend/themes/twentyeleven</a>),
	    H5Bp Annex themes are designed to serve as a Parent theme
...
/*	Define Annex URI */
	define('H5BP_ANNEX_URL', get_template_directory_uri());
	define('H5BP_ANNEX_CHILD_URL', get_stylesheet_directory_uri());
...
/*	1)	Add "H5Bp Annex Admin" link to left-nav Admin Menu & callback function for clicking that menu link */
		...
		if ( ! function_exists( 'H5BP_Annex_build_annex_admin_page' ) ):
			function H5BP_Annex_build_annex_admin_page() {
			?>
				<div id="annex-options-wrap">
					...
					<p>The clumsiest part of this plug-in is dealing with the CSS files.  Check the <a href="<?php echo H5BP_ANNEX_URL ?>/readme.txt">Read Me file</a> for details on how I suggest handling them.</p>
					<p>Choose below which options you want included in your site.</p>
					<form id="annex-options-form" method="post" action="options.php" enctype="multipart/form-data">
					...
					<script>
						(function(window){
							window.H5BP = {
								...
								init : function(){
									H5BP.form = jQuery('#annex-options-form');
		...
		endif; // H5BP_Annex_build_annex_admin_page

/*	2)	Add Admin Page CSS if on the Admin Page */

		if ( ! function_exists( 'H5BP_Annex_admin_register_head' ) ):
			function H5BP_Annex_admin_register_head() {
				echo '<link rel="stylesheet" href="' .H5BP_ANNEX_URL. '/annex-admin/admin-style.css">'.PHP_EOL;
			}
		endif; // H5BP_Annex_admin_register_head
		add_action('admin_head', 'H5BP_Annex_admin_register_head');


/*	3)	Add "H5Bp Annex Admin" Page options */
		...
		if ( ! function_exists( 'H5BP_Annex_register_and_build_fields' ) ):
			function H5BP_Annex_register_and_build_fields() {
				...
				add_settings_field('H5BP_Annex_favicon', 'Got Favicon?:', 'H5BP_Annex_favicon_setting', 'annex-admin', 'main_section');
				add_settings_field('H5BP_Annex_favicon_ithing', 'Got <em><abbr title="iPhone, iTouch, iPad...">iThing</abbr></em> Favicon?', 'H5BP_Annex_favicon_ithing_setting', 'annex-admin', 'main_section');
				add_settings_field('H5BP_Annex_modernizr_js', 'Modernizr JS?:', 'H5BP_Annex_modernizr_js_setting', 'annex-admin', 'main_section');
				add_settings_field('H5BP_Annex_respond_js', 'Respond JS?:', 'H5BP_Annex_respond_js_setting', 'annex-admin', 'main_section');
				add_settings_field('H5BP_Annex_jquery_js', 'jQuery JS?:', 'H5BP_Annex_jquery_js_setting', 'annex-admin', 'main_section');
		...
		endif; // H5BP_Annex_register_and_build_fields
		add_action('admin_init', 'H5BP_Annex_register_and_build_fields');

		//	Add Admin Page validation
		if ( ! function_exists( 'H5BP_Annex_validate_setting' ) ):
			function H5BP_Annex_validate_setting($plugin_options) {
		...
		endif; // H5BP_Annex_validate_setting

		//	in case you need it...
		if ( ! function_exists( 'H5BP_Annex_section_cb' ) ):
			function H5BP_Annex_section_cb() {
				// i don't do anything here, but you could if you wanted...
			}
		endif; // H5BP_Annex_section_cb

		//	callback fn for H5BP_Annex_google_chrome
		if ( ! function_exists( 'H5BP_Annex_google_chrome_setting' ) ):
			function H5BP_Annex_google_chrome_setting() {
				$options = get_option('plugin_options');
				$checked = (isset($options['H5BP_Annex_google_chrome']) && $options['H5BP_Annex_google_chrome']) ? 'checked="checked" ' : '';
				echo '<input class="check-field" type="checkbox" name="plugin_options[H5BP_Annex_google_chrome]" value="true" ' .$checked. '/>';
		...
		//	callback fn for H5BP_Annex_google_verification
		if ( ! function_exists( 'google_verification_setting' ) ):
			function H5BP_Annex_google_verification_setting() {
				$options = get_option('plugin_options');
				$checked = (isset($options['H5BP_Annex_google_verification']) && $options['H5BP_Annex_google_verification'] && $options['H5BP_Annex_google_verification_account'] && $options['H5BP_Annex_google_verification_account'] !== 'XXXXXXXXX...') ? 'checked="checked" ' : '';
				$account = (isset($options['H5BP_Annex_google_verification_account']) && $options['H5BP_Annex_google_verification_account']) ? $options['H5BP_Annex_google_verification_account'] : 'XXXXXXXXX...';
				$msg = ($account === 'XXXXXXXXX...') ? ', where </code>XXXXXXXXX...</code> will be replaced with the code you insert above' : '';
				echo '<input class="check-field" type="checkbox" name="plugin_options[H5BP_Annex_google_verification]" value="true" ' .$checked. '/>';
				...
				echo '<input type="text" size="40" name="plugin_options[H5BP_Annex_google_verification_account]" value="'.$account.'" onfocus="javascript:if(this.value===\'XXXXXXXXX...\'){this.select();}"></p>';
		...
		endif; // H5BP_Annex_google_verification_setting

		//	callback fn for H5BP_Annex_viewport
		if ( ! function_exists( 'H5BP_Annex_viewport_setting' ) ):
			function H5BP_Annex_viewport_setting() {
				$options = get_option('plugin_options');
				$checked = (isset($options['H5BP_Annex_viewport']) && $options['H5BP_Annex_viewport']) ? 'checked="checked" ' : '';
				$setting = (isset($options['H5BP_Annex_viewport_setting']) && $options['H5BP_Annex_viewport_setting']) ? $options['H5BP_Annex_viewport_setting'] : 'width=device-width';
				echo '<input class="check-field" type="checkbox" name="plugin_options[H5BP_Annex_viewport]" value="true" ' .$checked. '/>';
				...
				echo '<p><input type="text" size="40" name="plugin_options[H5BP_Annex_viewport_setting]" value="'.$setting.'"></p>';
				echo '<p>Selecting this option will add the following code to the <code class="html">&lt;head&gt;</code> of your pages:</p>';
				echo '<code>&lt;meta name=<span>"viewport"</span> content=<span>"'.$setting.'"</span>&gt;</code>';
			}
		endif; // H5BP_Annex_viewport_setting

		//	callback fn for H5BP_Annex_favicon
		if ( ! function_exists( 'H5BP_Annex_favicon_setting' ) ):
			function H5BP_Annex_favicon_setting() {
				$options = get_option('plugin_options');
				$checked = (isset($options['H5BP_Annex_favicon']) && $options['H5BP_Annex_favicon']) ? 'checked="checked" ' : '';
				echo '<input class="check-field" type="checkbox" name="plugin_options[H5BP_Annex_favicon]" value="true" ' .$checked. '/>';
		...
		endif; // H5BP_Annex_favicon_setting

		//	callback fn for H5BP_Annex_favicon_ithing
		if ( ! function_exists( 'H5BP_Annex_favicon_ithing_setting' ) ):
			function H5BP_Annex_favicon_ithing_setting() {
				$options = get_option('plugin_options');
				$checked = (isset($options['H5BP_Annex_favicon_ithing']) && $options['H5BP_Annex_favicon_ithing']) ? 'checked="checked" ' : '';
		...
		endif; // H5BP_Annex_favicon_ithing_setting

		//	callback fn for H5BP_Annex_modernizr_js
		if ( ! function_exists( 'H5BP_Annex_modernizr_js_setting' ) ):
			function H5BP_Annex_modernizr_js_setting() {
				$options = get_option('plugin_options');
				$checked = (isset($options['H5BP_Annex_modernizr_js']) && $options['H5BP_Annex_modernizr_js']) ? 'checked="checked" ' : '';
				echo '<input class="check-field" type="checkbox" name="plugin_options[H5BP_Annex_modernizr_js]" value="true" ' .$checked. '/>';
		...
		//	callback fn for H5BP_Annex_respond_js
		if ( ! function_exists( 'H5BP_Annex_respond_js_setting' ) ):
			function H5BP_Annex_respond_js_setting() {
				$options = get_option('plugin_options');
				$checked = (isset($options['H5BP_Annex_respond_js']) && $options['H5BP_Annex_respond_js']) ? 'checked="checked" ' : '';
				echo '<input class="check-field" type="checkbox" name="plugin_options[H5BP_Annex_respond_js]" value="true" ' .$checked. '/>';
				...
				echo '<p>Selecting this option will add the following code to the <code class="html">&lt;head&gt;</code> of your pages (note the lack of a version, when you\'re ready to upgrade, simply copy/paste the new version into the file below, and your site is ready to go!):</p>';
				echo '<code class="comment">&lt;!--[if lt IE 9]&gt;&lt;script src="' .H5BP_ANNEX_URL. '/js/vendor/respond.js"&gt;&lt;/script&gt;&lt;![endif]--&gt;</code>';
			}
		endif; // respond_js_setting

		//	callback fn for H5BP_Annex_jquery_js
		if ( ! function_exists( 'H5BP_Annex_jquery_js_setting' ) ):
			function H5BP_Annex_jquery_js_setting() {
				...
				$version = (isset($options['H5BP_Annex_jquery_version']) && $options['H5BP_Annex_jquery_version'] && $options['H5BP_Annex_jquery_version'] !== '') ? $options['H5BP_Annex_jquery_version'] : '1.9.0';
				...
				echo '<code><b>&lt;</b>script<b>&gt;</b><span class="support">window</span>.<b>jQuery</b> <span class="html">||</span> <span class="support">document</span>.write<b>(</b>\'<span>&lt;script src="'.H5BP_ANNEX_URL.'/js/vendor/jquery.js"&gt;&lt;</span><b>\/</b><span>script&gt;\'</span><b>)&lt;/</b>script<b>&gt;</b></code>';
				echo '<p><input class="check-field" type="checkbox" name="plugin_options[H5BP_Annex_jquery_head]" value="true" ' .$inhead. '/>';
				...
				echo '<input type="text" size="6" name="plugin_options[H5BP_Annex_jquery_version]" value="'.$version.'"> (<a href="http://code.google.com/apis/libraries/devguide.html#jquery">see all versions available via Google\'s CDN</a>)</p>';
			}
		endif; // H5BP_Annex_jquery_js_setting

		//	callback fn for H5BP_Annex_plugins_js
		if ( ! function_exists( 'H5BP_Annex_plugins_js_setting' ) ):
			function H5BP_Annex_plugins_js_setting() {
				$options = get_option('plugin_options');
				$checked = (isset($options['H5BP_Annex_plugins_js']) && $options['H5BP_Annex_plugins_js']) ? 'checked="checked" ' : '';
				echo '<input class="check-field" type="checkbox" name="plugin_options[H5BP_Annex_plugins_js]" value="true" ' .$checked. '/>';
				echo '<p>If you would like to use any <a href="http://plugins.jquery.com/">jQuery plug-ins</a>, H5Bp Annex provides a starter file located in:</p>';
				echo '<code>' .H5BP_ANNEX_URL. '/js/plugins.js</code>';
		...
		endif; // H5BP_Annex_plugins_js_setting

		//	callback fn for H5BP_Annex_site_js
		if ( ! function_exists( 'H5BP_Annex_site_js_setting' ) ):
			function H5BP_Annex_site_js_setting() {
				$options = get_option('plugin_options');
				$checked = (isset($options['H5BP_Annex_site_js']) && $options['H5BP_Annex_site_js']) ? 'checked="checked" ' : '';
				$inhead = (isset($options['H5BP_Annex_site_head']) && $options['H5BP_Annex_site_head']) ? 'checked="checked" ' : '';
				echo '<input class="check-field" type="checkbox" name="plugin_options[H5BP_Annex_site_js]" value="true" ' .$checked. '/>';
				echo '<p>If you would like to add your own site JavaScript file, H5Bp Annex provides a starter file located in:</p>';
				echo '<code>' .H5BP_ANNEX_URL. '/js/main.js</code>';
				echo '<p>This allows you to maintain your own code that will not get overwritten during Theme updates.</p>';
				echo '<p>Selecting this option will add the following code to your pages just before the <code>&lt;/body&gt;</code>:</p>';
				echo '<code><b>&lt;</b>script src<b>=</b><span>\'' .H5BP_ANNEX_CHILD_URL. '/js/main.js\'</span><b>&gt;&lt;</b>/script<b>&gt;</b></code>';
				echo '<p>(The single quotes and no-longer-necessary attributes are from WP, would like to fix that... maybe next update...)</p>';
				echo '<p><input class="check-field" type="checkbox" name="plugin_options[H5BP_Annex_site_head]" value="true" ' .$inhead. '/>';
		...
		endif; // H5BP_Annex_site_js_setting

		//	callback fn for H5BP_Annex_chrome_frame
		if ( ! function_exists( 'H5BP_Annex_chrome_frame_setting' ) ):
			function H5BP_Annex_chrome_frame_setting() {
				$options = get_option('plugin_options');
				$checked = (isset($options['H5BP_Annex_chrome_frame']) && $options['H5BP_Annex_chrome_frame']) ? 'checked="checked" ' : '';
				echo '<input class="check-field" type="checkbox" name="plugin_options[H5BP_Annex_chrome_frame]" value="true" ' .$checked. '/>';
		...
		endif; // H5BP_Annex_chrome_frame_setting

		//	callback fn for H5BP_Annex_google_analytics_js
		if ( ! function_exists( 'H5BP_Annex_google_analytics_js_setting' ) ):
			function H5BP_Annex_google_analytics_js_setting() {
				$options = get_option('plugin_options');
				$checked = (isset($options['H5BP_Annex_google_analytics_js']) && $options['H5BP_Annex_google_analytics_js'] && isset($options['H5BP_Annex_google_analytics_account']) && $options['H5BP_Annex_google_analytics_account'] && $options['H5BP_Annex_google_analytics_account'] !== 'XXXXX-X') ? 'checked="checked" ' : '';
				$account = (isset($options['H5BP_Annex_google_analytics_account']) && $options['H5BP_Annex_google_analytics_account']) ? str_replace('UA-','',$options['H5BP_Annex_google_analytics_account']) : 'XXXXX-X';
				$msg = ($account === 'XXXXX-X') ? ', where </code>XXXXX-X</code> will be replaced with the code you insert above' : '';
				echo '<input class="check-field" type="checkbox" name="plugin_options[H5BP_Annex_google_analytics_js]" value="true" ' .$checked. '/>';
				...
				echo 'UA-<input type="text" size="6" name="plugin_options[H5BP_Annex_google_analytics_account]" value="'.$account.'" onfocus="javascript:if(this.value===\'XXXXX-X\'){this.select();}" /></p>';
		...
		endif; // H5BP_Annex_google_analytics_js_setting


		//	callback fn for H5BP_Annex_search_form
		if ( ! function_exists( 'H5BP_Annex_search_form_setting' ) ):
			function H5BP_Annex_search_form_setting() {
				$options = get_option('plugin_options');
				$checked = (isset($options['H5BP_Annex_search_form']) && $options['H5BP_Annex_search_form']) ? 'checked="checked" ' : '';
				$placeholder = (isset($options['H5BP_Annex_search_placeholder_text']) && $options['H5BP_Annex_search_placeholder_text']) ? $options['H5BP_Annex_search_placeholder_text'] : '';
				echo '<input class="check-field" type="checkbox" name="plugin_options[H5BP_Annex_search_form]" value="true" ' .$checked. '/>';
				...
				echo '<input type="text" size="10" name="plugin_options[H5BP_Annex_search_placeholder_text]" value="'.$placeholder.'"></p>';
		...
		endif; // H5BP_Annex_search_form_setting

		//	callback fn for H5BP_Annex_cache_buster
		if ( ! function_exists( 'H5BP_Annex_cache_buster_setting' ) ):
			function H5BP_Annex_cache_buster_setting() {
				$options = get_option('plugin_options');
				$checked = (isset($options['H5BP_Annex_cache_buster']) && $options['H5BP_Annex_cache_buster']) ? 'checked="checked" ' : '';
				$version = (isset($options['H5BP_Annex_cache_buster_version']) && $options['H5BP_Annex_cache_buster_version']) ? $options['H5BP_Annex_cache_buster_version'] : '1';
				echo '<input class="check-field" type="checkbox" name="plugin_options[H5BP_Annex_cache_buster]" value="true" ' .$checked. '/>';
				...
				echo '<input type="text" size="4" name="plugin_options[H5BP_Annex_cache_buster_version]" value="'.$version.'"></p>';
		...
		endif; // H5BP_Annex_cache_buster_setting

		//	callback for footer credit
		if ( ! function_exists( 'H5BP_Annex_footer_credit_setting' ) ):
			function H5BP_Annex_footer_credit_setting() {
				$options = get_option('plugin_options');
				$checked = (isset($options['H5BP_Annex_footer_credit']) && $options['H5BP_Annex_footer_credit']) ? 'checked="checked" ' : '';
				...
				echo '<input class="check-field" type="checkbox" name="plugin_options[H5BP_Annex_footer_credit]" value="true" ' .$checked. '/>';
		...
		endif; // H5BP_Annex_footer_credit_setting


/*	4)	Create functions to add above elements to pages */

		//	$options['H5BP_Annex_google_chrome']
		if ( ! function_exists( 'H5BP_Annex_add_google_chrome' ) ):
			function H5BP_Annex_add_google_chrome() {
				echo '<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">'.PHP_EOL;
			}
		endif; // H5BP_Annex_add_google_chrome

		//	$options['H5BP_Annex_google_verification']
		if ( ! function_exists( 'H5BP_Annex_add_google_verification' ) ):
			function H5BP_Annex_add_google_verification() {
				$options = get_option('plugin_options');
				$account = $options['H5BP_Annex_google_verification_account'];
				echo '<meta name="google-site-verification" content="'.$account.'">'.PHP_EOL;
			}
		endif; // H5BP_Annex_add_google_verification

		//	$options['H5BP_Annex_viewport']
		if ( ! function_exists( 'H5BP_Annex_add_viewport' ) ):
			function H5BP_Annex_add_viewport() {
				$options = get_option('plugin_options');
				$setting = (isset($options['H5BP_Annex_viewport_setting']) && $options['H5BP_Annex_viewport_setting']) ? $options['H5BP_Annex_viewport_setting'] : 'width=device-width';
				echo '<meta name="viewport" content="'.$setting.'">'.PHP_EOL;
			}
		endif; // H5BP_Annex_add_viewport

		//	$options['H5BP_Annex_favicon']
		if ( ! function_exists( 'H5BP_Annex_add_favicon' ) ):
			function H5BP_Annex_add_favicon() {
				echo '<link rel="shortcut icon" href="/favicon.ico">'.PHP_EOL;
			}
		endif; // H5BP_Annex_add_favicon

		//	$options['H5BP_Annex_favicon_ithing']
		if ( ! function_exists( 'H5BP_Annex_add_favicon_ithing' ) ):
			function H5BP_Annex_add_favicon_ithing() {
		...
		endif; // H5BP_Annex_add_favicon_ithing

		//	$options['H5BP_Annex_modernizr_js']
		if ( ! function_exists( 'H5BP_Annex_add_modernizr_script' ) ):
			function H5BP_Annex_add_modernizr_script() {
				...
				echo '<script src="' .H5BP_ANNEX_URL. '/js/vendor/modernizr.js'.$cache.'"></script>'.PHP_EOL;
			}
		endif; // H5BP_Annex_add_modernizr_script

		//	$options['ieshiv_script']
		if ( ! function_exists( 'H5BP_Annex_add_ieshiv_script' ) ):
			function H5BP_Annex_add_ieshiv_script() {
				$cache = H5BP_Annex_cache_buster();
				echo '<!--[if lt IE 9]>'.PHP_EOL;
				echo '<script src="//html5shiv.googlecode.com/svn/trunk/html5.js"></script>'.PHP_EOL; // try getting from CDN
				echo '<script>window.html5 || document.write(\'<script src="' .H5BP_ANNEX_URL. '/js/ieshiv.js'.$cache.'"><\/script>\')</script>'.PHP_EOL; // fallback to local if CDN fails
				echo '<![endif]-->'.PHP_EOL;
			}
		endif; // H5BP_Annex_add_ieshiv_script

		//	$options['H5BP_Annex_respond_js']
		if ( ! function_exists( 'H5BP_Annex_add_respond_script' ) ):
			function H5BP_Annex_add_respond_script() {
				$cache = H5BP_Annex_cache_buster();
				echo '<!--[if lt IE 9]><script src="' .H5BP_ANNEX_URL. '/js/vendor/respond.js'.$cache.'"></script><![endif]-->'.PHP_EOL;
			}
		endif; // H5BP_Annex_add_respond_script

		//	$options['H5BP_Annex_jquery_js']
		if ( ! function_exists( 'H5BP_Annex_add_jquery_script' ) ):
			function H5BP_Annex_add_jquery_script() {
				$cache = H5BP_Annex_cache_buster();
				$options = get_option('plugin_options');
				$version = ($options['H5BP_Annex_jquery_version']) ? $options['H5BP_Annex_jquery_version'] : '1.9.0';
		...
		endif; // H5BP_Annex_add_jquery_script

		//	$options['H5BP_Annex_plugins_js']
		if ( ! function_exists( 'H5BP_Annex_add_plugin_script' ) ):
			function H5BP_Annex_add_plugin_script() {
				$cache = H5BP_Annex_cache_buster();
				echo '<script src="' .H5BP_ANNEX_CHILD_URL. '/js/plugins.js'.$cache.'"></script>'.PHP_EOL;
			}
		endif; // H5BP_Annex_add_plugin_script

		//	$options['H5BP_Annex_site_js']
		if ( ! function_exists( 'H5BP_Annex_add_site_script' ) ):
			function H5BP_Annex_add_site_script() {
				$cache = H5BP_Annex_cache_buster();
				echo '<script src="' .H5BP_ANNEX_CHILD_URL. '/js/main.js'.$cache.'"></script>'.PHP_EOL;
			}
		endif; // H5BP_Annex_add_site_script

		//	$options['H5BP_Annex_search_form']
		if ( ! function_exists( 'H5BP_Annex_search_form' ) ):
			function H5BP_Annex_search_form( $form ) {
				$options = get_option('plugin_options');
				$placeholder = $options['H5BP_Annex_search_placeholder_text'];
		...
		endif; // H5BP_Annex_search_form

		//	$options['H5BP_Annex_chrome_frame']
		if ( ! function_exists( 'H5BP_Annex_add_chrome_frame' ) ):
			function H5BP_Annex_add_chrome_frame() {
		...
		endif; // H5BP_Annex_add_chrome_frame

		//	$options['H5BP_Annex_google_analytics_js']
		if ( ! function_exists( 'H5BP_Annex_add_google_analytics_script' ) ):
			function H5BP_Annex_add_google_analytics_script() {
				$options = get_option('plugin_options');
				$account = $options['H5BP_Annex_google_analytics_account'];
		...
		endif; // H5BP_Annex_add_google_analytics_script

		//	$options['H5BP_Annex_cache_buster']
		if ( ! function_exists( 'H5BP_Annex_cache_buster' ) ):
			function H5BP_Annex_cache_buster() {
				$options = get_option('plugin_options');
				return (isset($options['H5BP_Annex_cache_buster']) && $options['H5BP_Annex_cache_buster']) ? '?ver='.$options['H5BP_Annex_cache_buster_version'] : '';
			}
		endif; // H5BP_Annex_cache_buster

		//	$options['H5BP_Annex_footer_credit']
		if ( ! function_exists( 'H5BP_Annex_add_footer_credit' ) ):
			function H5BP_Annex_add_footer_credit() {
				$options = get_option('plugin_options');
				$blog_title = get_bloginfo();
		...
		endif; // H5BP_Annex_add_footer_credit



/*	5)	Add H5Bp Annex options to page as requested */
		if (!is_admin() ) {
			...
			if (isset($options['H5BP_Annex_google_chrome']) && $options['H5BP_Annex_google_chrome']) {
				add_action('wp_print_styles', 'H5BP_Annex_add_google_chrome');
			}

			if (isset($options['H5BP_Annex_google_verification']) && $options['H5BP_Annex_google_verification'] && $options['H5BP_Annex_google_verification_account'] && $options['H5BP_Annex_google_verification_account'] !== 'XXXXXXXXX...') {
				add_action('wp_print_styles', 'H5BP_Annex_add_google_verification');
			}

			if (isset($options['H5BP_Annex_viewport']) && $options['H5BP_Annex_viewport']) {
				add_action('wp_print_styles', 'H5BP_Annex_add_viewport');
			}

			if (isset($options['H5BP_Annex_favicon']) && $options['H5BP_Annex_favicon']) {
				add_action('wp_print_styles', 'H5BP_Annex_add_favicon');
			}

			if (isset($options['H5BP_Annex_favicon_ithing']) && $options['H5BP_Annex_favicon_ithing']) {
				add_action('wp_print_styles', 'H5BP_Annex_add_favicon_ithing');
			}

			if (isset($options['H5BP_Annex_modernizr_js']) && $options['H5BP_Annex_modernizr_js']) {
				add_action('wp_print_styles', 'H5BP_Annex_add_modernizr_script');
			} else {
				// if Modernizr isn't selected, add IEShiv inside an IE Conditional Comment
				add_action('wp_print_styles', 'H5BP_Annex_add_ieshiv_script');
			}

			if (isset($options['H5BP_Annex_respond_js']) && $options['H5BP_Annex_respond_js']) {
				add_action('wp_print_styles', 'H5BP_Annex_add_respond_script');
			}

			if (isset($options['H5BP_Annex_jquery_js']) && $options['H5BP_Annex_jquery_js'] && isset($options['H5BP_Annex_jquery_version']) && $options['H5BP_Annex_jquery_version'] && $options['H5BP_Annex_jquery_version'] !== '') {
				// check if should be loaded in <head> or at end of <body>
				$hook = (isset($options['H5BP_Annex_jquery_head']) && $options['H5BP_Annex_jquery_head']) ? 'wp_print_styles' : 'wp_footer';
				add_action($hook, 'H5BP_Annex_add_jquery_script');
			}

			// for jQuery plug-ins, jQuery must also be set
			if (isset($options['H5BP_Annex_plugins_js']) && $options['H5BP_Annex_plugins_js']) {
				add_action('wp_footer', 'H5BP_Annex_add_plugin_script');
			}

			if (isset($options['H5BP_Annex_site_js']) && $options['H5BP_Annex_site_js']) {
				// check if should be loaded in <head> or at end of <body>
				$hook = (isset($options['H5BP_Annex_site_head']) && $options['H5BP_Annex_site_head']) ? 'wp_print_styles' : 'wp_footer';
				add_action($hook, 'H5BP_Annex_add_site_script');
			}

			if (isset($options['H5BP_Annex_search_form']) && $options['H5BP_Annex_search_form']) {
				add_filter( 'get_search_form', 'H5BP_Annex_search_form');
			}			

			if (isset($options['H5BP_Annex_chrome_frame']) && $options['H5BP_Annex_chrome_frame']) {
				add_action('ie_chrome_frame', 'H5BP_Annex_add_chrome_frame');
			}

			if (isset($options['H5BP_Annex_google_analytics_js']) && $options['H5BP_Annex_google_analytics_js'] && isset($options['H5BP_Annex_google_analytics_account']) && $options['H5BP_Annex_google_analytics_account'] && $options['H5BP_Annex_google_analytics_account'] !== 'XXXXX-X') {
				add_action('wp_footer', 'H5BP_Annex_add_google_analytics_script');
			}

			if (isset($options['H5BP_Annex_footer_credit']) && $options['H5BP_Annex_footer_credit'] && $options['H5BP_Annex_footer_credit'] !== '' ) {
				add_action('annex_credits', 'H5BP_Annex_add_footer_credit');
			}


		} // if (!is_admin() )

/*	End customization for H5Bp Annex */
